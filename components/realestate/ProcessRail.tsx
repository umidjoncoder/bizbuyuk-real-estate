"use client";

import { useRef } from "react";
import { motion, useScroll, useTransform, useReducedMotion } from "motion/react";
import { Reveal } from "../Reveal";

type Step = { n: string; title: string; lead: string; body: string };

/** Numbered rail that draws itself as the section scrolls past. */
export function ProcessRail({ title, lead, steps }: { title: string; lead: string; steps: Step[] }) {
  const railRef = useRef<HTMLOListElement>(null);
  const reduce = useReducedMotion();
  const { scrollYProgress } = useScroll({ target: railRef, offset: ["start 80%", "end 70%"] });
  const drawn = useTransform(scrollYProgress, [0, 1], ["0%", "100%"]);

  return (
    <div className="mx-auto max-w-[1280px] px-5 py-24 sm:px-8 sm:py-32">
      <Reveal>
        <h2 className="display max-w-[16ch] text-[clamp(2rem,5vw,3.4rem)]">{title}</h2>
      </Reveal>
      <Reveal delay={0.08}>
        <p className="mt-5 max-w-[56ch] text-[1rem] leading-relaxed text-muted">{lead}</p>
      </Reveal>

      <ol ref={railRef} className="relative mt-14 ps-10 sm:ps-14">
        <span aria-hidden className="absolute start-[7px] top-3 bottom-3 w-px bg-line sm:start-[11px]" />
        <motion.span
          aria-hidden
          style={{ height: reduce ? "100%" : drawn }}
          className="absolute start-[7px] top-3 w-px bg-gradient-to-b from-champagne to-bronze sm:start-[11px]"
        />
        {steps.map((step, i) => (
          <li key={step.n} className="relative pb-11 last:pb-0">
            <motion.span
              aria-hidden
              initial={reduce ? false : { scale: 0.3, opacity: 0.25 }}
              whileInView={{ scale: 1, opacity: 1 }}
              viewport={{ once: true, margin: "-35% 0px -35% 0px" }}
              transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }}
              className="absolute -start-10 top-2 block h-[15px] w-[15px] rounded-full bg-gold ring-4 ring-ink sm:-start-14 sm:h-[23px] sm:w-[23px]"
            />
            <Reveal delay={i * 0.04}>
              <div className="grid gap-x-10 gap-y-2 md:grid-cols-[6rem_1fr]">
                <p className="display text-[1.5rem] text-gold-foil">{step.n}</p>
                <div className="min-w-0">
                  <h3 className="text-lg font-extrabold tracking-tight">{step.title}</h3>
                  <p className="mt-1.5 max-w-[46ch] text-[0.98rem] leading-relaxed text-cream/85">{step.lead}</p>
                  <p className="mt-1.5 max-w-[62ch] text-[0.9rem] leading-relaxed text-muted">{step.body}</p>
                </div>
              </div>
            </Reveal>
          </li>
        ))}
      </ol>
    </div>
  );
}
