// World country dial codes for the international phone field.
// Flags are derived from the ISO‑3166 alpha‑2 code (regional indicator symbols),
// so there are no hard‑coded emoji to mistype.

export type Country = {
  code: string; // ISO 3166-1 alpha-2 (e.g. "UZ")
  name: string; // English display name
  dial: string; // calling code incl. "+" (e.g. "+998")
};

// Turn "UZ" into 🇺🇿 using Unicode regional-indicator letters.
...
// Comprehensive list of sovereign states + common territories with dial codes.
...
const PRIMARY_FOR_DIAL: Record<string, string> = {
  "+7": "RU", // RU / KZ
  "+1": "US", // US / CA / NANP (longer +1NPA codes still win below)
  "+44": "GB", // GB / IM / JE / GG
  "+39": "IT", // IT / VA
};

// Best-effort: split a stored phone like "+998 90 123 45 67" back into a known
// country (longest matching dial prefix wins) + the national remainder.
export function splitPhone(raw: string | null | undefined): { country: Country; national: string } {
  const s = String(raw || "").trim();
  if (s.startsWith("+")) {
    const digits = "+" + s.slice(1).replace(/\D/g, "");
    // Find the LONGEST dial code this number begins with, so "+1671" beats "+1".
    let best: Country | null = null;
    for (const c of COUNTRIES) {
      if (digits.startsWith(c.dial) && (!best || c.dial.length > best.dial.length)) best = c;
    }
    if (best) {
      const primaryCode = PRIMARY_FOR_DIAL[best.dial];
      const chosen = (primaryCode && COUNTRIES.find((c) => c.code === primaryCode)) || best;
      return { country: chosen, national: digits.slice(best.dial.length) };
    }
  }
  return { country: DEFAULT_COUNTRY, national: s.replace(/^\+/, "") };
}
